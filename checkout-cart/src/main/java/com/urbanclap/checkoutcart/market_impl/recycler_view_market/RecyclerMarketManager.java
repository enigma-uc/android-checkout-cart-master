...
public class RecyclerMarketManager<IT, NT, CT> extends MarketManager<IT, NT, CT> implements CartCallback<CT> {
    ...
    @NonNull
    List<Section<IT>> sections2;
    @NonNull
    RecyclerMarketView marketView;
    @NonNull
    ...
    public RecyclerMarketManager(@NonNull Context context,
                                 ...
                                 @NonNull RecyclerItemFactory<IT, CT> itemFactory, @NonNull RecyclerMarketView marketView) {


        this(config, MarketUtils.getDefaultRecycler(context, false), itemFactory, marketView);
    }
    ...
    public void handleNavigateTo(@NonNull String id) {
        if (id.equals("fire"))
        {
            handleAddSectionsWithItemChanged(this.sections2);
            return;
        }
    ...
    public void handleAddSectionsWithItemChanged(@NonNull List<Section<IT>> sectionList) {
        for (Section<IT> section : sectionList) {
            if (itemPool.hasSection(section.getId())) {
                Pair<Integer, Integer> removePair = itemPool.removeWithoutFirstItem(section.getId());
                adapter.notifyItemRangeRemoved(removePair.first+1, removePair.second - (removePair.first+1));
                Pair<Integer, Integer> addPair = itemPool.add2(section, removePair.first+1);
                adapter.notifyItemRangeInserted(addPair.first, (addPair.second - addPair.first));
            } else {
                Pair<Integer, Integer> addPair = itemPool.add(section);
                adapter.notifyItemRangeInserted(addPair.first+1, addPair.second - addPair.first);
            }
        }
    }
    ...
    public void updateSections2(@NonNull List<Section<IT>> sections2) {
